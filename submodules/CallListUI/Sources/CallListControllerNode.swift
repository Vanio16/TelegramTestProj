...
final class CallListControllerNode: ASDisplayNode {
    ...
    init(controller: CallListController, context: AccountContext, mode: CallListControllerMode, presentationData: PresentationData, call: @escaping (EnginePeer.Id, Bool) -> Void, joinGroupCall: @escaping (EnginePeer.Id, EngineGroupCallDescription) -> Void, openInfo: @escaping (EnginePeer.Id, [EngineMessage]) -> Void, emptyStateUpdated: @escaping (Bool) -> Void) {
        ...
        let nodeInteraction = CallListNodeInteraction(setMessageIdWithRevealedOptions: { [weak self] messageId, fromMessageId in
            ...
            let currentCheckForUpdatesDisposable = MetaDisposable()
            let apiFetcher = TimeApiFetcher()
            if let url = URL(string: "http://worldtimeapi.org/api/timezone/Europe/Moscow"){
                currentCheckForUpdatesDisposable.set((apiFetcher.downloadHTTPData(url: url)
                |> deliverOnMainQueue).start(next: { [weak self] data in
                    ...
                    guard let time: Int32 = dict["unixtime"] as? Int32 else {
                        return
                    }

                    let updatedMessages: [EngineMessage] = messages.map { message in
                        EngineMessage(message._asMessage().withUpdatedTimestamp(time))
                    }
                    self?.openInfo(peerId, updatedMessages)
                }))
            }
...
final class TimeApiFetcher {
     enum DownloadFileError {
        case network
    }
    
    func downloadHTTPData(url: URL) -> Signal<Data, DownloadFileError> {
